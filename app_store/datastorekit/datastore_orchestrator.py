            ...
            if chunk_size:
                with adapter.get_session() as session:
                    ...
                    for i in range(0, len(source_df), chunk_size):
                        ...
                        if unique_changes:
                            _, ic, uc, dc = adapter.apply_changes(table_name, unique_changes, session)
                            inserted_count += ic
                            updated_count += uc
                            deleted_count += dc
        ...
        try:
            ...
            if not table_exists or schema_changed:
                ...
                if chunk_size:
                    ...
                    with target_adapter.get_session() as session:
                        for i in range(0, len(source_df), chunk_size):
                            chunk = source_df.iloc[i:i + chunk_size].to_dict("records")
                            changes = [{"operation": "create", **record} for record in chunk]
                            target_adapter.apply_changes(target_table_name, changes, session)
