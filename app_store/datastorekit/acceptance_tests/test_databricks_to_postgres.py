# datastorekit/acceptance_tests/databricsk_to_pstgre_test
import os
...
def test_databricks_to_postgres_replication():
    ...
    # Verify history table (full load)
...
def test_replication_error_handling():
    ...
    env_path = os.path.join(".env", ".env.databricks_dev")
    assert Config.validate_env_file(env_path, "databricks"), f"Invalid Databricks .env file: {env_path}"
    env_path = os.path.join(".env", ".env.postgres_dev")
    assert Config.validate_env_file(env_path, "postgres"), f"Invalid PostgreSQL .env file: {env_path}"
    ...
    test_data = [
        {"unique_id": 1, "secondary_key": "A", "category": "Food", "amount": 50.0},
        {"unique_id": 1, "secondary_key": "A", "category": "Travel", "amount": 100.0}  # Duplicate key
    ]
